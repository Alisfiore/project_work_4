...
for new_cat, old_cats in new_categories_map.items():
    for old_cat in old_cats:
        mapping_list.append({"old_category": old_cat, "new_category": new_cat})
...
# Creazione della tabella nel database PostgreSQL
with psycopg.connect(host=host, dbname=dbname, user=user, password=password, port=port) as conn:
    with conn.cursor() as cur:
        ...
        sql = """
        INSERT INTO categories (new_category) 
        SELECT %s 
        WHERE NOT EXISTS (
            SELECT 1 FROM categories WHERE new_category = %s
        );
        """
        for index, row in df_final.iterrows():
            cur.execute(sql, (row["new_category"], row["new_category"]))
